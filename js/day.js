...
l214 = ['玫瑰与爱意永不凋零💘', '愿有情人终成眷属', '今天宜表白！']
if (m == 2 && dd == 14) {//妇女节
    if (sessionStorage.getItem("isPopupWindow") != "1") {
        Swal.fire(l214[Math.floor(Math.random() * l.length)]);
        sessionStorage.setItem("isPopupWindow", "1");
    }
}
if (m == 3 && dd == 8) {//妇女节
...
l520 = ['爱要勇敢说出来💌', '今天适合说"我爱你"', '执子之手与子偕老💑',"今年是520情人节\n快和你喜欢的人一起过吧！💑"]
if (m == 5 && dd == 20) {//520
    if (sessionStorage.getItem("isPopupWindow") != "1") {
        Swal.fire(l520[Math.floor(Math.random() * l.length)]);
        sessionStorage.setItem("isPopupWindow", "1");
    }
}
if (m == 7 && dd == 1) {//建党节
